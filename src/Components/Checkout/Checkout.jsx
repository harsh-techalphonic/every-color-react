...
export default function Checkout() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const dataaddress = response.data?.gst_response || [];
        setBillingAddresses(dataaddress);
      } catch (err) {
  ...
  const submitCheckOut = async (e) => {
    ...
    const payload = {
      ...
      platform: "Website",
      order_amount: finalTotal,
      coupon_code: isCouponApplied ? coupon : null,
      products: derivedCartData.map((item) => ({
        product_id: item?.id,
        customization: item?.customization || null,
        product_quantity: item?.quantity,
  ...
  useEffect(() => {
    ...
    const calculatedTax = TOTAL_TAX; // total gst from all products
  ...
  <div className="mt-4">
    ...
    <div className="card p-3 shadow-sm border">
      ...
      <div className="d-flex justify-content-between">
        <h6 className="mb-2">
          {billingAddresses.taxpayerInfo.lgnm}
        </h6>
      </div>
      ...
      <p className="mb-1">
        <strong>GSTIN:</strong>{" "}
        {billingAddresses.taxpayerInfo.gstin}
      </p>

      {/* Address Line 1 */}
      <p className="mb-1">
        {billingAddresses.taxpayerInfo.pradr?.addr?.flno},{" "}
        {billingAddresses.taxpayerInfo.pradr?.addr?.bno},{" "}
        {billingAddresses.taxpayerInfo.pradr?.addr?.st}
      </p>

      {/* Address Line 2 */}
      <p className="mb-1">
        {billingAddresses.taxpayerInfo.pradr?.addr?.loc},{" "}
        {billingAddresses.taxpayerInfo.pradr?.addr?.dst}
      </p>

      {/* State & Pincode */}
      <p className="mb-0 text-muted">
        {billingAddresses.taxpayerInfo.pradr?.addr?.stcd} -{" "}
        {billingAddresses.taxpayerInfo.pradr?.addr?.pncd}
      </p>
    </div>
  </div>
)}
          ...
          <div className="col-md-4">
            {/* Coupon Box */}
            <div
              className="discount_box mb-3 mt-4 w-100"
              style={{ maxWidth: "100%" }}
            >
                ...
                <tbody>
                  <tr>
                    ...
                    <td className="text-end">
                      ₹{checkoutDetail.subTotal.toFixed(2)}
                    </td>
                  </tr>
                  <tr>
                    <td>Discount</td>
                    <td className="text-end">
                      - ₹{checkoutDetail.productDiscount.toFixed(2)}
                    </td>
                  </tr>
                  <tr>
                    <td>Coupon</td>
                    <td className="text-end">
                      - ₹{checkoutDetail.couponDiscount.toFixed(2)}
                    </td>
                  </tr>
                  ...
                  <tr>
                    ...
                    <td className="text-end">
                      {" "}
                      (GST) ₹{checkoutDetail.tax.toFixed(2)}
                    </td>
                  </tr>
